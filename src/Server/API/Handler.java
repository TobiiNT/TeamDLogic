...
import Server.Utilities.ByteConverter;
...
public abstract class Handler {
    ...
    protected void Execute(HttpExchange exchange) throws Exception {
        ...
        if (RequestMethodAllowed.equals(exchange.getRequestMethod())) {
            ResponseEntity e = DoPost(ByteConverter.Convert(exchange.getRequestBody()));
    ...
    protected abstract <T> T DoPost(String input) throws Exception;

    protected <T> T ReadRequest(String input, Class<T> type) {
        return Try.of(() -> ObjectMapper.readValue(input, type)).getOrElseThrow(ApplicationExceptions.invalidRequest());
    }
